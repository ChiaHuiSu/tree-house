...
namespace Treehierarchy
{
    ...
    public:
        ...
        //bool enable_reorderAccumulation = false;
        bool enable_reorder_feature = false;
        bool enable_reorder_threshold = false;
        ...
        bool enable_threshold_dedupe = false;
    };
    ...
    public:
        ...
        ModuleOp lowerToLLVMModule()
        {
            ...
            m_context.getOrLoadDialect<cf::ControlFlowDialect>();
            ...
            if (m_option.enable_reorder)
            {
                pm.addPass(tree::createTreeReorder(tree::TreeReorderOptions({.isFLIntEnabled = m_option.enable_flint,
                                                                             .useFeature = m_option.enable_reorder_feature,
                                                                             .useThreshold = m_option.enable_reorder_threshold})));
            }
            //pm.enableIRPrinting();
            //llvm::DebugFlag = true;
            ...
            if (m_option.enable_inline)
            {
                ...
                pm.addPass(tree::createTreeInline(tree::TreeInlineOptions({.pinningAmount = m_option.regNum})));
            }
            ...
            pm.addPass(tree::createTreeLowering(tree::TreeLoweringOptions({.thresholdOrdering = m_option.enable_threshold_dedupe})));
            ...
            /*std::error_code ec;
            llvm::raw_fd_ostream filestream("/home/ywshao/hello2.mlir", ec);
            filestream << *m_module;
            filestream.flush();
            llvm::errs() << "TEST: Dump to hello.mlir\n";
            exit(0);*/
    ...
    protected:
        ...
        void buildNodeOp(Block *entryBlock, DecisionTree *tree, int idx)
        {
            ...
            if (!node.IsLeaf())
            {
                Value feature = m_builder.create<tree::FloatFeatureOp>(loc, entryBlock->getArgument(0), node.featureIndex);
