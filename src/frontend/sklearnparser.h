...
namespace Treehierarchy
{
    ...
    void SklearnParser::CreatePredictFunction()
    {
        ...
        auto functionType = m_builder.getFunctionType({getPointerType(), getPointerType()}, std::nullopt);
        ...
        for (size_t i = 0; i < m_forest->GetTreeSize(); i++)
        {
            ...
            m_builder.create<func::CallOp>(loc, StringRef("tree_" + std::to_string(i)), std::nullopt, operands);
        }
